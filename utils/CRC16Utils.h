/**
 * File name: CRC16Utils.h
 * Author: JoshuaWong
 * Email: 1034316376@qq.com
 * Version: 1.0
 * Created on: Jun 28, 2017 11:56:21 AM
 * Description: 
 */

#ifndef UTILS_CRC16UTILS_H_
#define UTILS_CRC16UTILS_H_

#include "../common.h"

//与CRC16校验有关的变量
const uint8_t CRC_H[] = { 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0, 0x80, 0x41,
...
const uint8_t CRC_L[] = { 0x00, 0xC0, 0xC1, 0x01, 0xC3, 0x03, 0x02, 0xC2, 0xC6, 0x06, 0x07, 0xC7,
...
namespace CRC16Utils {
inline uint16_t CRC16(uint8_t *buf, uint8_t len);
}

namespace CRC16Utils {
uint16_t CRC16(uint8_t *buf, uint8_t len) {
	uint8_t i;
	uint8_t crcH = 0xff;	// 初始化高字节
	uint8_t crcL = 0xff;	// 初始化低字节
	uint16_t crcRet;
	uint8_t index;

	for (i = 0; i < len; i++) {
		index = crcH ^ buf[i];   //计算CRC

		crcH = crcL ^ CRC_H[index];
		crcL = CRC_L[index];
		crcRet = (crcH << 8) | crcL;
	}
	return crcRet;
